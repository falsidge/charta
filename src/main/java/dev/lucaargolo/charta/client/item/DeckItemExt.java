...
@OnlyIn(Dist.CLIENT)
public class DeckItemExt extends BlockEntityWithoutLevelRenderer {

    private static final RandomSource RANDOM = RandomSource.create();

    public DeckItemExt(BlockEntityRenderDispatcher blockEntityRenderDispatcher, EntityModelSet entityModelSet) {
    ...
    public void renderByItem(@NotNull ItemStack stack, @NotNull ItemDisplayContext displayContext, @NotNull PoseStack poseStack, @NotNull MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        List<BakedQuad> transformedQuads = model.getQuads(null, null, RANDOM).stream().map(DeckItemExt::replaceQuadSprite).toList();
