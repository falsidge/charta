...
public class FunScreen extends GameScreen<FunGame, FunMenu> {
    ...
    public void containerTick() {
        ...
        if(!menu.isCurrentPlayer()) {
            ...
            if(menu.getCarriedCards().isEmpty()) {
                Charta.INSTANCE.sendToServer(new CardContainerSlotClickPayload(menu.containerId, menu.cardSlots.size()-3, -1));
            }else{
                drawAll = false;
            }
            Charta.INSTANCE.sendToServer(new CardContainerSlotClickPayload(menu.containerId, menu.cardSlots.size()-1, -1));
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int y = height - ((int) CardSlot.getHeight(CardSlot.Type.HORIZONTAL))/2 - 14;
        if(mouseX >= x && mouseX < x+65 && mouseY >= y && mouseY < y+18) {
            Charta.INSTANCE.sendToServer(new LastFunPayload());
            return true;
        }
        x += width/2 + ((int) CardSlot.getWidth(CardSlot.Type.HORIZONTAL))/2;
        if(mouseX >= x && mouseX < x+65 && mouseY >= y && mouseY < y+18) {
...
//        float[] colors = nextPlayer.getColor().getTextureDiffuseColors();
        int color = PortUtils.fromArrayColors(nextPlayer.getColor().getTextureDiffuseColors());
        text = Component.translatable("message.charta.next_player", nextPlayer.getName()).withStyle(s -> s.withColor(PortUtils.fromArrayColors(nextPlayer.getColor().getTextureDiffuseColors())));
        ...
        if(menu.isGameReady()) {
            if (menu.isCurrentPlayer()) {
                text = Component.translatable("message.charta.your_turn").withStyle(s -> s.withColor(PortUtils.fromArrayColors(player.getColor().getTextureDiffuseColors())));
            } else {
                text = Component.translatable("message.charta.other_turn", player.getName()).withStyle(s -> s.withColor(PortUtils.fromArrayColors(player.getColor().getTextureDiffuseColors())));
            }
